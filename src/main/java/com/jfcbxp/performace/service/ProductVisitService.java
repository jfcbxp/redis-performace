...
public class ProductVisitService {
    ...
    private Mono<Void> updateBatch(Map<Integer,Long> map){
        ...
        RScoredSortedSetReactive<Integer> sortedSet = batch.getScoredSortedSet("product:visit:" + format, IntegerCodec.INSTANCE);
        return Flux.fromIterable(map.entrySet())
                .flatMap(e -> sortedSet.addScore(e.getKey(),e.getValue()))
                .then(batch.execute())
                .then();
